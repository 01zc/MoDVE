...
main <- function() {
  ...
  output <- foreach(
    numPool = seq(numSpeciesPools[1], numSpeciesPools[2]),
    .export = c(
      "ComputeSuitabilityUnscaled",
      "int_seq",
      "Parabol",
      "SuitabilityScore"
    )
    ...
    SpeciesPool <- read.csv(
      file.path(DirectorySpeciesPools, SpeciesPoolFileName),
      sep = ",",
      header = TRUE
    )
    ...
    if (!is.na(singleStep)) {
      ...
      print(
        paste0(
          "Computing suitability scores for species pool ",
          numPool,
          "for each variable ..."
        )
      )
      ...
      FileNamemicrohabitat <- file.path(Directorymicrohabitat,
                                        paste0("microhabitatMatrix", t, ".rds"))
      if (!file.exists(FileNamemicrohabitat)) {
        stop(
          paste(
            "microhabitat file for time step",
            t,
            "does not exist:",
            FileNamemicrohabitat
          )
        )
      }
      ...
      for (j in seq_along(EnvScoreVars)) {
        ...
        for (i in seq_len(nrow(SpeciesPool))) {
          ...
          if (VarName == "Light" & LightResponseFct == "Parabolic") {
            EnvVarSuit <- Parabol(
              SpeciesPool$LightResponseA[i],
              SpeciesPool$LightResponseB[i],
              SpeciesPool$LightResponseC[i],
              envVar
            )
          } else {
      ...
      if (sum(nicheFlags) > 1) { # more than one variable
      ...
      maxSuitFile <- file.path(DirectoryOutputSpeciesPool, paste0("GlobalMaxSuitability_", numPool, ".h5"))
      ...
      if (file.exists(maxSuitFile)) {
        ...
        print(
          paste0(
            "Computing max. suitability scores for species pool ",
            numPool,
            " for each species ..."
          )
        )
        ...
        tryCatch({
          ...
          rhdf5::h5write(globalMaxSuitability,
                         maxSuitFile,
                         "GlobalMaxSuitability")
        }, error = function(e) {
      ...
      print(
        paste0(
          "Recompute combined scores and scale them for species pool ",
          numPool,
          " ..."
        )
      )
      ...
      for (step in 0:timeSteps) {
        ...
        writeLines(paste0(
          "Step",
          t,
          ": Avg. Scaled Suitability =",
          round(avgScaledSuitability, 3),
          "\n"
        ))
        ...
        tryCatch({
          ...
          rhdf5::h5write(as.array(scaledSuitability),
                         outFile,
                         "ScaledSuitabilityScores")
          ...
          if (!"ScaledSuitabilityScores" %in% contents$name) {
            ...
            print(contents)
            stop(paste0(
              "Dataset 'ScaledSuitabilityScores' not found in: ",
              envSuitPath
            ))
          }
